...
public abstract class AbstractApplicationContext
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			prepareRefresh();
			/**
			 * TODO 2.获取Bean工厂，期间会做解析和加载bean定义的一些列工作.生成BeanDefinition对象.
			 *       此处返回的beanFactory的真实类型为：DefaultListableBeanFactory
			 *       主要工作：1.初始化beanFactory，2.解析文件获取BeanDefinition
			 *
			 *
			...
			/**
			 * TODO  3.bean工厂的初始化准备工作，设置bean工厂的一些属性
			 *       主要工作：1.注册一个Aware类型的BeanFactoryPostProcessor，后续会用到
			...
			try {
				 ...
				 *
				 * TODO 该方法在Bean定义加载完毕之后，Bean实例化之前会执行
				...
				/**
				 * TODO 5.执行beanFactory的后置处理器,执行时机，加载完BeanDefinition后，暂未开始实例化
				 *
				 * TODO (1)先执行BeanDefinitionRegistryPostProcessor接口的实现类的postProcessBeanDefinitionRegistry方法，
				 *   具体是先执行ConfigurationClassPostProcessor，springboot的自动装配就是在这里干的
				 ...
				 *
				 * TODO (2)然后执行BeanFactoryPostProcessor接口的实现类的postProcessBeanFactory方法
				 ...
				 *
				 * TODO 注册所有实现了BeanPostProcessor接口的后置处理器   注意：这里只是注册，并没有实际调用其方法
				 ...
				 *
				 * TODO SpringBoot也是在改步骤中启动内嵌Tomcat容器的
				 */
				onRefresh();
				...
				/** 12.TODO 发布事件。例如容器中的刷新事件:ContextRefreshedEvent就是在这一步中发布.
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// TODO 添加bean的后置处理器。此处添加的是Spring自己的后置处理器，用来回调bean所实现的aware接口中的方法.
		...
		*/
		...
		// TODO 补充：1、增加对AspectJ的支持，在Java中织入分为3种：(1) 编译期织入; (2) 类加载期织入; (3) 运行期织入。编译期织入指的是在java编译期，
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//TODO  缓存(冻结)所有的BeanName（注册的bean定义不会被修改或进一步做处理了，因为下面马上要创建Bean的实例对象了）
		beanFactory.freezeConfiguration();

		//TODO  初始化所有的单实例Bean，包括创建单实例bean的全部过程
